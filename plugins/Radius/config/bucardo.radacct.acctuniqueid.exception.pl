...
##
## Radacct exception handler
##
## To add this script to a sync, assuming the setting of the variables in the
## next section, run this command, replacing `radius` with the name of the
## sync it applies to.
##
##     bucardo add customcode radacct_acctuniqueid_key_conflict \
##            whenrun=exception \
##            src_code=bucardo.radacct.acctuniqueid.exception.pl \
##            sync=radius \
##            getdbh=1
...
my $relation = 'public.radacct'; # index schema
my $index    = 'radacct_acctuniqueid_key';   # index name
my $pk_col   = 'radacctid';              # index table primary key column
my $time_col = 'acctupdatetime';      # last update time column
