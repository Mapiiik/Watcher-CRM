<?php
/**
 * @var \App\View\AppView $this
 * @var iterable<\Radius\Model\Entity\Radreply> $radreplies
 */
?>
<div class="radreply index content">
    <?= $this->AuthLink->link(
        __d('radius', 'New RADIUS Reply'),
        ['action' => 'add'],
        ['class' => 'button float-right win-link'],
    ) ?>
    <?= $this->heading(__d('radius', 'RADIUS Replies')) ?>
    <div class="table-responsive">
        <table>
            <thead>
                <tr>
                    <th><?= $this->Paginator->sort('username') ?></th>
                    <th><?= $this->Paginator->sort('attribute') ?></th>
                    <th><?= $this->Paginator->sort('op') ?></th>
                    <th><?= $this->Paginator->sort('value') ?></th>
                    <th class="actions"><?= __d('radius', 'Actions') ?></th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($radreplies as $radreply) : ?>
                <tr>
                    <td>
                        <?= $radreply->account !== null ? $this->Html->link(
                            $radreply->account->username,
                            ['controller' => 'Accounts', 'action' => 'view', $radreply->account->id],
                        ) : $radreply->username ?>
                    </td>
                    <td><?= h($radreply->attribute) ?></td>
                    <td><?= h($radreply->op) ?></td>
                    <td><?= h($radreply->value) ?></td>
                    <td class="actions">
                        <?= $this->AuthLink->link(__d('radius', 'View'), ['action' => 'view', $radreply->id]) ?>
                        <?= $this->AuthLink->link(
                            __d('radius', 'Edit'),
                            ['action' => 'edit', $radreply->id],
                            ['class' => 'win-link'],
                        ) ?>
                        <?= $this->AuthLink->postLink(
                            __d('radius', 'Delete'),
                            ['action' => 'delete', $radreply->id],
                            ['confirm' => __d('radius', 'Are you sure you want to delete # {0}?', $radreply->id)],
                        ) ?>
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
    </div>
    <?= $this->element('Radius.common/paginator') ?>
</div>
