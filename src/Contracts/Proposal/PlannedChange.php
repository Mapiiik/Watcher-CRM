...
/**
 * One field applying the changes will write, and what it will write over.
 *
 * Read twice: once by the preview, which draws it, and once by applying the changes, which does it. That
...
{
    /**
     * @param string $target Which record - one of {@see \App\Contracts\Proposal\ChangePlan}'s.
     ...
     * @param bool $asked Whether somebody asked for this, or it follows from applying the papers.
     */
    ...
    public function agenda(): string
    {
        return match ($this->target) {
            ChangePlan::CONTRACT => __('Contract'),
            ChangePlan::VERSION, ChangePlan::REPLACED_VERSION => __('Contract Version'),
            default => $this->target,
        };
    }
     ...
     *
     * Still written, and still shown: what is worth knowing is that applying the changes touches the
