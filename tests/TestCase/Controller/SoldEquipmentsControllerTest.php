...
class SoldEquipmentsControllerTest extends TestCase
{
    ...
    public function testAddUnderTheRouteFilesItUnderTheRoute(): void
    {
        ...
        $before = $this->idsIn('SoldEquipments');
        $this->post('/customers/' . self::CUSTOMER_ID . '/contracts/' . self::CONTRACT_ID . '/sold-equipments/add', [
            'equipment_type_id' => $this->firstId('EquipmentTypes'),
        ]);

        $this->assertRedirect();
        $added = $this->addedRecord('SoldEquipments', $before);
